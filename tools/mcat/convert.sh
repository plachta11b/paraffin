...
read -r -d '' run_script << "EOM"
	...
	result_files="$(find /data/motifs/ -name "results.html")"
	...
	for result_dir in ${result_dirs}; do
		cd ${result_dir}
		for result_file in results.html; do
		...
		cd ..
		first="true"
		for file in $(find . -name "results.html.meme"); do
			if [ $first = "true" ]; then
