...
read -r -d '' run_script << "EOM"
	...
	result_files="$(find /data/motifs/ -name "$match")"
	...
	for result_dir in ${result_dirs}; do
		...
		for result_file in PPM-*.txt; do
			...
			python -m lead2gold \
				...
				--motif-out meme \
				--file-in "${file}" \
				--file-out "${file/.txt/.txt.meme}"
		done

		# todo implement multiple motif merging in python instead of this
		# meme2meme $cwd/*.meme > $cwd/result_converted.pwm
		cd /data/motifs/$(echo ${result_dir/\/data\/motifs\//} | sed 's|/.*||')
		first="true"
		for file in $(find . -name "$match.meme"); do
			if [ $first = "true" ]; then
